...
<body class="hold-transition skin-blue sidebar-mini">
					...
					Quản lý Danh mục <small>
						<form action="addCategory">
							<button class="button button1" type="submit"
								value="Thêm Danh Mục">
								<strong>Thêm danh mục mới </strong><i class="fa fa-plus"></i>
							</button>
						</form>
					</small>
				</h1>
					...
					<tbody>
						<c:forEach var="category" items="${categories}">
							<tr>
								...
								<td>${category.name}</td>
								<td><a
									href="category/${category.id}/delete?user=${user.id}"
									onclick="return confirm('Bạn chắc chắn muốn xóa ?')"><button
											class="btn btn-danger" type="button">
											<span class="glyphicon glyphicon-trash">
										</button></a></td>
								<td><a
									href="editCategory?cateID=${category.id}&&user=${user.id}"><button
	...
	<footer
		...
		<strong>Copyright &copy; 2019 | <a
			href="https://www.facebook.com">TTH SHOP</a>.
		</strong>
	</footer>
	<!-- footer -->
	<!-- /.content -->
